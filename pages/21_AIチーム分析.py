...
with st.form("feedback_form"):
    ...
    ret1m = st.number_input("1ヶ月後リターン(%)", value=0.0, step=0.1)
    ...
    if submit:
        with engine.begin() as con:
            con.execute(
                text(
                    """
                    INSERT INTO agent_feedback(created_at, run_id, ticker, ai_recommendation, human_decision, human_reason, action_taken, actual_return_1m)
                    VALUES(:created_at, :run_id, :ticker, :ai_rec, :decision, :reason, :action_taken, :ret)
                    """
                ),
                {
