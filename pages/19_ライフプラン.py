...
c1.metric("FIRE達成確率", f"{mc['fire_probability'] * 100:.1f}%")
c2.metric("FIRE達成年齢(中央値)", "未達" if mc["fire_age_median"] is None else f"{int(mc['fire_age_median'])}歳")
...
with tabs[0]:
    ...
    table_df = det_df[["age", "assets", "annual_cashflow", "annual_return_amount", "one_time_expense", "is_fire_phase"]].copy()
    table_df.columns = ["年齢", "資産(円)", "年間収支(円)", "年間リターン(円)", "一時イベント支出(円)", "FIREフェーズ"]
    st.dataframe(table_df.style.format({"資産(円)": "¥{:,.0f}", "年間収支(円)": "¥{:,.0f}", "年間リターン(円)": "¥{:,.0f}", "一時イベント支出(円)": "¥{:,.0f}"}), use_container_width=True, height=320)

with tabs[1]:
    st.markdown("#### プリセットイベント")
    preset_cols = st.columns(4)
    presets = [
        ("💍 結婚", 35, EVENT_TYPE_ONE_TIME_EXPENSE, 3_500_000, 1),
        ("👶 出産", 36, EVENT_TYPE_ONE_TIME_EXPENSE, 500_000, 1),
        ("🏠 住宅ローン", 40, EVENT_TYPE_RECURRING_EXPENSE, 1_440_000, 35),
        ("🎓 大学", 56, EVENT_TYPE_RECURRING_EXPENSE, 1_000_000, 4),
        ("🚗 車購入", 42, EVENT_TYPE_ONE_TIME_EXPENSE, 2_500_000, 1),
        ("🏥 介護", 60, EVENT_TYPE_RECURRING_EXPENSE, 1_000_000, 5),
        ("📈 昇給", 40, EVENT_TYPE_INCOME_CHANGE, 500_000, 1),
        ("🏖️ 年金開始", 65, EVENT_TYPE_INCOME_CHANGE, pension_annual, 1),
    ]
    ...
    with st.form("event_add_form"):
        ...
        amount = ec4.number_input("金額(円)", min_value=0, value=500_000, step=100_000)
        duration = ec5.number_input("期間(年, 継続支出用)", min_value=1, max_value=50, value=1)
...
with tabs[4]:
    ...
    plan_payload = {
        ...
        "fire_deterministic": {
            "fire_achieved": bool(det["fire_achieved"]),
            "fire_age": det["fire_age"],
            "fire_target_now": fire_target_now,
        },
        "monte_carlo": {
            "fire_probability": mc["fire_probability"],
            "fire_age_median": mc["fire_age_median"],
            "fire_age_p10": mc["fire_age_p10"],
            "fire_age_p90": mc["fire_age_p90"],
        },
    ...
    st.markdown("#### 講座間受け渡しファイル")
    if st.button("long_term_plan_inputs.csv を保存", use_container_width=True):
        out_df = pd.DataFrame(
            [
                {
                    "current_age": current_age,
        ...
        out_path = Path(__file__).resolve().parents[1] / "long_term_plan_inputs.csv"
        out_df.to_csv(out_path, index=False, encoding="utf-8-sig")
        st.success(f"保存しました: {out_path}")

    if st.button("automation_policy.md を保存", use_container_width=True):
        md = f"""# Automation Policy (Generated)

- generated_at: {datetime.now().isoformat(timespec='seconds')}
- fire_probability: {mc['fire_probability']:.2%}
- median_fire_age: {mc['fire_age_median']}

## Human vs AI Decision Boundary
- AI: シミュレーション計算と改善案の提案
- Human: 実行可否判断、生活費前提の確定、最終投資判断
...
- 市場前提(期待リターン/ボラ)の変更
"""
        out_path = Path(__file__).resolve().parents[1] / "automation_policy.md"
