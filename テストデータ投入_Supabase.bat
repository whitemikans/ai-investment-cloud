...
set "DB_URL=%~1"
if "%DB_URL%"=="" (
  set /p DB_URL=Supabase DATABASE_URL (postgresql://...): 
)

if "%DB_URL%"=="" (
  echo DATABASE_URL is empty. Abort.
  pause
  exit /b 1
)

echo [SUPABASE] Seeding test data...
"%PY_EXE%" "scripts\seed_test_data_supabase.py" --database-url "%DB_URL%"
if %ERRORLEVEL% neq 0 (
  echo Failed. Check URL and connection.
  pause
  exit /b %ERRORLEVEL%
)
